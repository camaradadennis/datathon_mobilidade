...
# Classificadores de diagnóstico.
...
CID_VEICULOS = {
    'pedestre': r'V0[0-9]',
    'ciclista': r'V1[0-9]',
    'motociclista': r'V2[0-9]',
    'automovel': r'V4[0-9]',
    'caminhonete': r'V5[0-9]',
}

def veiculo_in_row(row):
    for veiculo, cid in CID_VEICULOS.items():
        if row.str.match(cid).any():
            return veiculo

    return 'indeterminado ou outros'
...
def extrair(de_arquivo):
    ...
    df['tipo_veiculo'] = df[cols_cid].apply(veiculo_in_row, axis=1)
